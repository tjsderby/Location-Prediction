...
locationData = locationData.iloc[::-1]
...
shiftSteps = int(1279 / 6)  # average daily records divided by 6
...
# 256 random sequences of sequenceLength from the data
batchSize = 256 # batch is dependent on GPU and RAM

# half the avg daily records
sequenceLength = int(1279 / 2) # amount of time steps the random batch will be
...
model.add(LSTM(units=128, return_sequences=True, input_shape=(None, numSignalsX,))) # adding 128 output signals

model.add(Dropout(0.2)) # adding dropout regularisation

model.add(LSTM(units=128, return_sequences=True))

model.add(Dropout(0.2)) # adding dropout regularisation

model.add(Dense(numSignalsY, activation='selu')) # dense layer to condense output signals down to 2
...
model.fit_generator(generator=generator, epochs=20, steps_per_epoch=100, validation_data=validationData, callbacks=callbacks) # this now trains our network
...
pltComp(startIndex=0, length=38251, train=True, graph=1)
longLatComp(startIndex=0, length=38251, train=True)

pltComp(startIndex=0, length=4251, train=False, graph=2)
longLatComp(startIndex=0, length=4251, train=False)